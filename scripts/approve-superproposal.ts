...
async function main() {
  ...
  if (
    (await superPod.getProposals({ status: 'queued' }))[0].status !== ('passed' || 'rejected') ||
    (await subPod.getProposals({ status: 'queued' }))[0].status !== ('passed' || 'rejected') ||
    (await subPodTwo.getProposals({ status: 'queued' }))[0].status !== ('passed' || 'rejected')
  ) {
    throw new Error(
